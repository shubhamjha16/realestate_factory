...
import config
from property_data_parser import parse_property_data
from valuation_calculator  import compute as val_compute
...
def _valuation_critic_route(state: REState) -> str:
    ...
    if state.get("structure_attempt", 0) >= config.MAX_CRITIC_RETRIES:
        return "section_drafter"
    return "valuation_structure"
...
def renderer_node(state: REState) -> dict:
    from re_renderer import render
    try:
        path = render(
...
def _renderer_route(state: REState) -> str:
    if state.get("render_error") and state.get("render_attempt", 0) < config.MAX_HEALER_RETRIES:
        return "healer"
    return "upload"
